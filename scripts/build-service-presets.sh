...
for file in "$SOURCE_DIR/services"/*.json; do
    ...
    # Keep all fields including metadata (name, description, category)
    config=$(jq '.' "$file")
...
echo ""
echo -e "${GREEN}🛠️  Loading reference tools:${NC}"
...
for file in "$SOURCE_DIR/reference-tools"/*.json; do
    ...
    # Keep all fields including metadata (name, description, category, language)
    config=$(jq '.' "$file")
...
# ============================================================================
# Generate subjects.js (needs to be done before preset-groups.js)
# ============================================================================
...
for file in "$SOURCE_DIR/subjects"/*.json; do
    ...
    if [ "$toolPresets" = "*" ]; then
        ...
        tool_array_for_subject="$ALL_TOOLS"
    else
        config=$(jq '{toolPresets}' "$file")
        tool_array_for_subject=$(jq '.toolPresets' "$file")
    fi
    
    # Add to SUBJECTS
    if [ "$first_subject" = true ]; then
    ...
    # Add to allowedTools (for PRESET_GROUPS)
    if [ "$first_allowed" = true ]; then
        ALLOWED_TOOLS_JSON="$ALLOWED_TOOLS_JSON\"$id\": $tool_array_for_subject"
        first_allowed=false
    else
        ALLOWED_TOOLS_JSON="$ALLOWED_TOOLS_JSON,\"$id\": $tool_array_for_subject"
    fi
...
# Now generate preset-groups.js with allowedTools
...
const PRESET_GROUPS = {
    ...
    "allowedTools": $(echo "$ALLOWED_TOOLS_JSON" | jq .)
};
EOF
...
# ============================================================================
echo -e "${GREEN}🖥️  Generating platform-specific files:${NC}"
...
echo -e "\n${GREEN}✅ Generated platform-specific files${NC}\n"

# ============================================================================
# Generate xml-data.js from example_config.xml
# ============================================================================
if [ -f "$SOURCE_DIR/example_config.xml" ]; then
    echo -e "${GREEN}📄 Generating xml-data.js from example_config.xml:${NC}"
...
EOF
    ...
    echo -e "  ✓ xml-data.js"
    echo -e "\n${GREEN}✅ Generated xml-data.js${NC}\n"
fi
