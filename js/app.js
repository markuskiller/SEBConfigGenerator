...
// VERSION & BUILD INFO
// ============================================================================
const APP_VERSION = 'v0.23.0b11';
const BUILD_DATE = new Date('2025-11-23T01:40:00'); // Format: YYYY-MM-DDTHH:mm:ss
...
checkbox.addEventListener('change', () => {
    ...
    syncSharePointPatternsToURLFilter();
    updatePreview();
});
...
header.addEventListener('click', async () => {
    ...
    if (isExpanding && !contentLoaded) {
        ...
        const manualCount = parsedDictStructures.urlFilterRules.filter(r => !r.autoGenerated).length;
        const autoGenCount = parsedDictStructures.urlFilterRules.filter(r => r.autoGenerated).length;
        const autoGenText = currentLang === 'de' 
            ? `+ ${autoGenCount} automatisch + ${getAllDomains().length} Domains` 
            : `+ ${autoGenCount} auto + ${getAllDomains().length} domains`;
        countSpan.textContent = `(${manualCount} ${currentLang === 'de' ? 'manuell' : 'manual'} ${autoGenText})`;
    }
...
// Mark auto-generated rules visually
if (rule.autoGenerated) {
    card.classList.add('auto-generated');
}
...
// Add label badge for auto-generated rules
const labelBadge = rule.autoGenerated 
    ? `<span class="auto-gen-badge" title="${currentLang === 'de' ? 'Automatisch generiert aus SharePoint-Einstellungen' : 'Auto-generated from SharePoint settings'}">🔄 ${rule.label || 'SharePoint'}</span>`
    : '';
    ...
    card.innerHTML = `
        ...
        <div class="url-filter-controls">
            ...
            <button class="btn-icon url-filter-delete" 
                    data-index="${index}"
                    title="${rule.autoGenerated ? (currentLang === 'de' ? 'Auto-generierte Regeln können nicht gelöscht werden' : 'Auto-generated rules cannot be deleted') : (currentLang === 'de' ? 'Regel löschen' : 'Delete rule')}"
                    ${rule.autoGenerated ? 'disabled' : ''}>
                ${rule.autoGenerated ? '🔒' : '🗑️'}
            </button>
        </div>
    `;// Event listeners (only for non-auto-generated rules)
if (!rule.autoGenerated) {
...
if (!rule.autoGenerated) {
    card.querySelector('.url-filter-delete').addEventListener('click', () => {
        if (confirm(currentLang === 'de' ? 'Regel wirklich löschen?' : 'Really delete this rule?')) {
            ...
            if (countSpan) {
                const ruleCount = parsedDictStructures.urlFilterRules.filter(r => !r.autoGenerated).length;
                const autoGenCount = parsedDictStructures.urlFilterRules.filter(r => r.autoGenerated).length;
                const autoGenText = currentLang === 'de' 
                    ? `+ ${autoGenCount} automatisch + ${getAllDomains().length} Domains` 
                    : `+ ${autoGenCount} auto + ${getAllDomains().length} domains`;
...
// Synchronize SharePoint patterns to URL filter rules (for UI display)
function syncSharePointPatternsToURLFilter() {
    // Remove existing SharePoint auto-generated rules
    parsedDictStructures.urlFilterRules = parsedDictStructures.urlFilterRules.filter(rule => 
        !rule.autoGenerated || rule.autoGenerated !== 'sharepoint'
    );
    
    // Get current SharePoint patterns
    const patterns = getSharePointUrlPatterns();
    
    // Add them as auto-generated rules (marked so they can be removed/updated)
    patterns.forEach(pattern => {
        parsedDictStructures.urlFilterRules.push({
            ...pattern,
            autoGenerated: 'sharepoint'
        });
    });
    ...
    debugLog(`✅ Synced ${patterns.length} SharePoint patterns to URL filter rules`);
}
...
// Add manual URL filter rules from advanced settings (exclude auto-generated)
parsedDictStructures.urlFilterRules
    .filter(rule => !rule.autoGenerated)
    .forEach(rule => {
        urlFilterRulesXML += `\t\t<dict>
\t\t\t<key>action</key>
...
document.getElementById('sharepointLink').addEventListener('input', function() {
    ...
    if (serviceType) {
        ...
        // Sync patterns to URL filter when link is parsed
        syncSharePointPatternsToURLFilter();
    }
});
